...
public class BatchService : IBatchService
{
    ...
    public BatchService(XtractDbContext context, ILogger<BatchService> logger, IConfiguration configuration)
    {
        _context = context;
        _logger = logger;
    ...
    public async Task<Result<List<BatchListResponse>>> GetAllAsync()
    {
        try
        {
            ...
            var batchListResponses = batches.Select(b => new BatchListResponse
            {
                ...
                ValidOrders = b.WorkItems.Count(w => w.Status != WorkItemStatus.ValidationError),
                InvalidOrders = b.WorkItems.Count(w => w.Status == WorkItemStatus.ValidationError),
    ...
    public async Task<Result<BatchResponse>> ReprocessBatchAsync(ReprocessBatchRequest request, int currentUserId)
    {
        try
        {
            ...
            {
                var errorOrders = await _context.WorkItems
                    .Where(w => w.BatchId == request.BatchId && w.Status == WorkItemStatus.ValidationError)
                    .ToListAsync();

                foreach (var order in errorOrders)
                {
                    order.Status = WorkItemStatus.Created;
    ...
    public async Task<Result<bool>> DeleteAsync(int id)
    {
        try
        {
            ...
            _context.WorkItems.RemoveRange(batch.WorkItems);
    ...
    private async Task<ProcessingResult> ProcessMetadataFile(Entities.Entities.Batch batch, IFormFile metadataFile, IFormFileCollection? documents)
    {
        ...
        try
        {
            ...
            {
                if (int.TryParse(mapping.SchemaFieldId, out var schemaFieldId))
                {
                    fieldMappingLookup[mapping.InputField.ToLowerInvariant()] = schemaFieldId;
                }
                else
                {
                    _logger.LogWarning("Invalid SchemaFieldId '{SchemaFieldId}' in field mapping for client {ClientId}",
                        mapping.SchemaFieldId, batch.ClientId);
                }
            }
                ...
                if (groupedRecord["metadata"] is Dictionary<string, object> metadata)
                {
                    foreach (var field in metadata)
                    {
                        ...
                        if (fieldMappingLookup.TryGetValue(inputFieldName, out var schemaFieldId))
                        {
                            ...
                            _context.WorkItemData.Add(orderData);
                        }
                        else
                        {
    ...
    private async Task<List<BatchValidationResult>> PerformBatchValidations(int batchId)
    {
        ...
        var orders = await _context.WorkItems
        ...
        foreach (var order in orders)
        {
            ...
            if (missingRequiredFields.Any())
            {
                ...
                order.Status = WorkItemStatus.ValidationError;
        ...
        foreach (var order in orders)
        {
            var hasDocuments = await _context.Documents
                .AnyAsync(d => d.WorkItemId == order.Id);

            if (!hasDocuments)
            {
                ordersWithMissingPDFs.Add(order.Id);
                if (order.Status != WorkItemStatus.ValidationError)
                {
                    order.Status = WorkItemStatus.ValidationError;
        ...
        foreach (var order in orders.Where(o => o.Status == WorkItemStatus.Created))
        {
            order.Status = WorkItemStatus.ReadyForAI;
        }
    ...
    private async Task<List<BatchValidationResult>> GetBatchValidationResults(int batchId)
    {
        ...
        var errorOrders = orders.Where(o => o.Status == WorkItemStatus.ValidationError).ToList();
        var readyOrders = orders.Where(o => o.Status == WorkItemStatus.ReadyForAI).ToList();
    ...
    {
        var validOrders = batch.WorkItems.Count(w => w.Status != WorkItemStatus.ValidationError);
        var invalidOrders = batch.WorkItems.Count(w => w.Status == WorkItemStatus.ValidationError);
